...
const sanitizeTextOrArray = (a, config) => {
  if (typeof a === "string") {
    return sanitizeText(a, config);
  }
  // TODO: Refactor to avoid flat.
  return a.flat().map((x) => sanitizeText(x, config));
};
...
const evaluate = (val) =>
  val === false ||
  ["false", "null", "0"].includes(String(val).trim().toLowerCase())
    ? false
    : true;
...
const isMathMLSupported = () => window.MathMLElement !== undefined;
...
const hasKatex = (text) => {
  ...
  return (
    ((_b =
      (_a = text.match(katexRegex)) === null || _a === void 0
        ? void 0
 ...
 */
const calculateMathMLDimensions = async (text, config) => {
 ...
 */
const renderKatex = async (text, config) => {
  if (!hasKatex(text)) {
    return text;
  }
  if (
    !(isMathMLSupported() || config.legacyMathML || config.forceLegacyMathML)
  ) {
    return text.replace(
      katexRegex,
