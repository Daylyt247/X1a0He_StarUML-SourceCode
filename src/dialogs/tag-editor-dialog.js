...
const dialogTemplate = fs.readFileSync(
  path.join(__dirname, "../static/html-contents/tag-editor-dialog.html"),
  "utf8",
);
...
class TagEditorDialog {
  ...
  toDataItem(tag, attr) {
    return {
      ...
      valueIcon:
        tag && tag.reference && tag.kind === "reference"
          ? tag.reference.getNodeIcon()
          : "",
    };
  }
  ...
  updateDataItem(dataItem, values) {
    ...
    dataItem.set(
      "valueIcon",
      values.reference && values.kind === "reference"
        ? values.reference.getNodeIcon()
        : null,
    );
  }
  ...
  updateDataSource(elem) {
    ...
    if (elem.stereotype instanceof type.UMLStereotype) {
      attrs = elem.stereotype.attributes.concat(
        elem.stereotype.getInheritedAttributes(),
      );
