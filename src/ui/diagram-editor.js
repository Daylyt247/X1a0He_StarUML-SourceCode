...
const { isHorz, distanceToLine } = require("../core/geometry");

const configJson = configUtils.getConfigJson();
const MAX_DIAGRAM_CANVAS_WIDTH =
  typeof configJson.diagramCanvasWidth === "number"
    ? configJson.diagramCanvasWidth
    : 4000;
const MAX_DIAGRAM_CANVAS_HEIGHT =
  typeof configJson.diagramCanvasHeight === "number"
    ? configJson.diagramCanvasHeight
    : 3000;
...
class ManipulatableNotifier {
  mouseMove(editor, canvas, e) {
    ...
    if (v instanceof NodeView) {
      ...
      if (
        b === true &&
        (v.sizable === NodeView.SZ_FREE || v.sizable === NodeView.SZ_RATIO)
      ) {
        if (
          Coord.equalPt2(
            new Point(
              zr.x1 - GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE,
              zr.y1 - GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE,
            ),
            p,
          ...
          Coord.equalPt2(
            new Point(
              zr.x2 + GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE,
              zr.y1 - GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE,
            ),
            p,
            GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE,
          ...
          Coord.equalPt2(
            new Point(
              zr.x1 - GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE,
              zr.y2 + GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE,
            ),
            p,
            GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE,
          )
      ...
      if (
        b === true &&
        (v.sizable === NodeView.SZ_FREE || v.sizable === NodeView.SZ_VERT)
      ) {
        if (
          Coord.equalPt2(
            new Point(mx, zr.y1 - GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE),
            p,
            GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE,
          )
        ) {
          ...
          Coord.equalPt2(
            new Point(mx, zr.y2 + GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE),
            p,
            GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE,
          )
        ) {
      ...
      if (
        b === true &&
        (v.sizable === NodeView.SZ_FREE || v.sizable === NodeView.SZ_HORZ)
      ) {
        if (
          Coord.equalPt2(
            new Point(zr.x1 - GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE, my),
            p,
            GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE,
          )
        ) {
          ...
          Coord.equalPt2(
            new Point(zr.x2 + GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE, my),
            p,
            GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE,
          )
        ) {
      ...
      // check mouse near segment
      let nearSegment = false;
      let nearSegmentType = "free";
      if (v.points.count() > 1) {
        for (let i = 1; i < v.points.count(); i++) {
          let sp = v.points.getPoint(i - 1).copy();
          let ep = v.points.getPoint(i).copy();
          Coord.coordTransform(
            canvas.origin,
            canvas.zoomFactor,
            GridFactor.NO_GRID,
          ...
          const d = distanceToLine(p, sp, ep);
          if (d < GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE) {
            nearSegment = true;
            if (
              v.lineStyle === EdgeView.LS_RECTILINEAR ||
              v.lineStyle === EdgeView.LS_ROUNDRECT
            ) {
              nearSegmentType = isHorz(sp, ep) ? "horz" : "vert";
            }
            break;
          }
        }
      ...
      if (v.points.count() > 2) {
        for (let i = 0; i < v.points.count(); i++) {
          ...
          if (
            Coord.equalPt2(hp, p, GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE)
          ) {
            inHandle = true;
            break;
          }
      ...
      if (Coord.equalPt2(zp1, p, GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE)) {
        ...
        Coord.equalPt2(zp2, p, GraphicUtils.DEFAULT_HALF_HIGHLIGHTER_SIZE)
      ) {
        editor.setCursor(CURSOR_HAND);
      } else if (inHandle) {
        editor.setCursor(CURSOR_MOVE);
      } else if (nearSegment) {
        if (nearSegmentType === "horz") {
          editor.setCursor(CURSOR_NS_RESIZE);
        } else if (nearSegmentType === "vert") {
          editor.setCursor(CURSOR_EW_RESIZE);
        } else {
          editor.setCursor(CURSOR_CROSSHAIR);
        }
      } else {
        editor.setCursor(CURSOR_DEFAULT);
      }
...
class ContainmentHandlingProxy {
  ...
  canBeContainedAt(view, targetView) {
    if (
      ...
      targetView === null ||
      view.model === null ||
      targetView.model === null
    ) {
      return false;
    }
    return (
      targetView.canContainView(view) && targetView.model.canContain(view.model)
    );
  }
...
class SelectHandler extends Handler {
  ...
  mouseDown(editor, canvas, e) {
    ...
    this.doubleClicked = false;
    if (
      editor.diagram.selectedViews.length === 1 &&
      this.isPointInSelectionLine(canvas, editor.diagram.selectedViews[0], x, y)
    ) {
      ...
      if (this.view === null) {
        ...
        if (editor.diagram.selectedViews.length > 0 && e.shiftDown) {
          ...
          if (this.manipulator instanceof NodeManipulator) {
            if (
              this.view.containerChangeable &&
              this.manipulator.selectedPtType === GraphicUtils.CT_ELSE
            ) {
          ...
          var vs = [];
          for (
            var i = 0, len = editor.diagram.selectedViews.length;
            i < len;
            i++
          ) {
    ...
    if (this.mode === SM_SELECT_AREA) {
      ...
      if (this.mode === SM_GROUPING) {
        ...
        if (this.boundingBox.x2 > editor.diagramWidth) {
          ...
          this.boundingBox.x1 =
            editor.diagramWidth - (this.boundingBox.x2 - this.boundingBox.x1);
        ...
        if (this.boundingBox.y2 > editor.diagramHeight) {
          ...
          this.boundingBox.y1 =
            editor.diagramHeight - (this.boundingBox.y2 - this.boundingBox.y1);
          this.boundingBox.y2 = editor.diagramHeight;
        }
        editor.drawRubberband(
          this.boundingBox.x1,
          this.boundingBox.y1,
          this.boundingBox.x2,
          this.boundingBox.y2,
        ...
        editor.containmentHandlingProxy.mouseMoveHandling2(
          editor,
          canvas,
          vs,
          v,
        ...
        if (this.manipulator instanceof NodeManipulator) {
          if (
            this.view.containerChangeable &&
            this.manipulator.selectedPtType === GraphicUtils.CT_ELSE &&
            (this.manipulator.f1.x !== this.manipulator.f2.x ||
              this.manipulator.f1.y !== this.manipulator.f2.y)
          ) {
            editor.containmentHandlingProxy.mouseMoveHandling(
              editor,
              canvas,
              this.view,
              v,
  ...
  mouseUp(editor, canvas, e) {
    ...
    if (this.mode === SM_SELECT_AREA) {
      ...
      if (this.g2.x !== this.g1.x || this.g2.y !== this.g1.y) {
        if (
          editor.containmentHandlingProxy.activated &&
          editor.diagram.selectedViews[0].containerView !==
            editor.containmentHandlingProxy.containerView
        ) {
          editor.changeSelectedViewsContainer(
            this.g2.x - this.g1.x,
            this.g2.y - this.g1.y,
            editor.containmentHandlingProxy.containerView,
          );
        } else {
          editor.moveSelectedViews(
            this.g2.x - this.g1.x,
            this.g2.y - this.g1.y,
          );
        }
...
class NodeManipulator extends Manipulator {
  ...
  resizeSkeletonLeft(canvas, delta) {
    ...
    if (this.left + delta.x >= this.minRect.x1) {
      ...
      this.originRect.x1 <= (_ref = this.left + delta.x) &&
      _ref <=
        this.gridFitX(canvas, this.originRect.x1) + canvas.gridFactor.width
    ) {
      ...
      delta.x =
        delta.x -
        (this.left + delta.x - this.gridFitX(canvas, this.left + delta.x));
  ...
  resizeSkeletonRight(canvas, delta) {
    var _ref;
    if (
      this.right + delta.x <= this.minRect.x2 ||
      this.gridFitX(canvas, this.right + delta.x) <= this.minRect.x2
    ) {
      delta.x = this.minRect.x2 - this.right;
      this.right = this.minRect.x2;
    } else if (
      this.originRect.x2 <= (_ref = this.right + delta.x) &&
      _ref <=
        this.gridFitX(canvas, this.originRect.x2) + canvas.gridFactor.width
    ) {
      ...
      delta.x =
        delta.x -
        (this.right + delta.x - this.gridFitX(canvas, this.right + delta.x));
  ...
  resizeSkeletonTop(canvas, delta) {
    ...
    if (this.top + delta.y >= this.minRect.y1) {
      ...
      this.originRect.y1 <= (_ref = this.top + delta.y) &&
      _ref <=
        this.gridFitY(canvas, this.originRect.y1) + canvas.gridFactor.height
    ) {
      ...
      delta.y =
        delta.y -
        (this.top + delta.y - this.gridFitY(canvas, this.top + delta.y));
  ...
  resizeSkeletonBottom(canvas, delta) {
    var _ref;
    if (
      this.bottom + delta.y <= this.minRect.y1 ||
      this.gridFitY(canvas, this.bottom + delta.y) <= this.minRect.y2
    ) {
      delta.y = this.minRect.y2 - this.bottom;
      this.bottom = this.minRect.y2;
    } else if (
      this.originRect.y2 <= (_ref = this.bottom + delta.y) &&
      _ref <=
        this.gridFitY(canvas, this.originRect.y2) + canvas.gridFactor.height
    ) {
      ...
      delta.y =
        delta.y -
        (this.bottom + delta.y - this.gridFitY(canvas, this.bottom + delta.y));
    ...
    switch (this.sizable) {
      ...
      case NodeView.SZ_RATIO:
        if (Coord.equalPt2(new Point(zr.x1 - HHS, zr.y1 - HHS), p, HHS)) {
          ...
          Coord.equalPt2(new Point(zr.x2 + HHS, zr.y1 - HHS), p, HHS)
        ) {
          this.selectedPtType = GraphicUtils.CT_RT;
        } else if (
          Coord.equalPt2(new Point(zr.x1 - HHS, zr.y2 + HHS), p, HHS)
        ) {
          this.selectedPtType = GraphicUtils.CT_LB;
        } else if (
          Coord.equalPt2(new Point(zr.x2 + HHS, zr.y2 + HHS), p, HHS)
        ) {
    ...
    switch (this.selectedPtType) {
      ...
      case GraphicUtils.CT_LM:
        this.minRect = new Rect(
          this.right - this.minX + 1,
          this.bottom - this.minY + 1,
          this.right,
          this.bottom,
        );
        break;
      ...
      case GraphicUtils.CT_RM:
        this.minRect = new Rect(
          this.left,
          this.top,
          this.left + this.minX - 1,
          this.top + this.minY - 1,
        );
        break;
      case GraphicUtils.CT_RT:
        this.minRect = new Rect(
...
class EdgeManipulator extends Manipulator {
  ...
  beginManipulate(editor, canvas, view, x, y) {
    ...
    if (
      this.style === EdgeView.LS_RECTILINEAR ||
      this.style === EdgeView.LS_ROUNDRECT
    ) {
      ...
      if (l.head && this.selectedIndex !== this.points.count() - 1) {
        this.points.setPoint(
          this.points.count() - 1,
          Coord.getCenter(l.head.getBoundingBox(canvas)),
        );
      }
      if (this.selectedIndex === -1) {
        this.selectedIndex = l.containedIndex(canvas, z);
    ...
    if (dx !== 0 || dy !== 0) {
      ...
      if (this.edgeSelectLocation === SL_LINE) {
        ...
        if (this.selectedIndex < this.points.count()) {
          v = editor.diagram.getViewAt(
            canvas,
            this.points.getPoint(this.selectedIndex).x,
            this.points.getPoint(this.selectedIndex).y,
            true,
          );
        }
    ...
    if (
      ...
      if (
        ...
        if (
          p1.x + delta.x >= op1.x &&
          p1.x + delta.x <=
            this.gridFitX(canvas, op1.x) + canvas.gridFactor.width
        ) {
          delta.x = op1.x - p1.x;
          p1.x = op1.x;
        } else {
          delta.x =
            delta.x - (p1.x + delta.x - this.gridFitX(canvas, p1.x + delta.x));
          p1.x = this.gridFitX(canvas, p1.x + delta.x);
        }
        if (
          p1.y + delta.y >= op1.y &&
          p1.y + delta.y <=
            this.gridFitY(canvas, op1.y) + canvas.gridFactor.height
        ) {
          delta.y = op1.y - p1.y;
          p1.y = op1.y;
        } else {
          delta.y =
            delta.y - (p1.y + delta.y - this.gridFitY(canvas, p1.y + delta.y));
        ...
        if (p1.x === p2.x) {
          if (
            op1.x <= (_ref2 = p1.x + delta.x) &&
            _ref2 <= this.gridFitX(canvas, op1.x) + canvas.gridFactor.width
          ) {
            delta.x = op1.x - p1.x;
            p1.x = op1.x;
          } else {
            delta.x =
              delta.x -
              (p1.x + delta.x - this.gridFitX(canvas, p1.x + delta.x));
            p1.x = this.gridFitX(canvas, p1.x + delta.x);
          }
          p2.x = p1.x;
          ...
          if (
            op1.y <= (_ref3 = p1.y + delta.y) &&
            _ref3 <= this.gridFitY(canvas, op1.y) + canvas.gridFactor.height
          ) {
            delta.y = op1.y - p1.y;
            p1.y = op1.y;
          } else {
            delta.y =
              delta.y -
              (p1.y + delta.y - this.gridFitY(canvas, p1.y + delta.y));
            p1.y = this.gridFitY(canvas, p1.y + delta.y);
          }
          p2.y = p1.y;
        }
      ...
      if (
        (this.edgeSelectLocation === SL_LINE && this.selectedIndex === 0) ||
        (this.edgeSelectLocation === SL_HEAD && this.points.count() === 2)
      ) {
        ...
        (this.edgeSelectLocation === SL_LINE &&
          this.selectedIndex === this.points.count() - 2) ||
        (this.edgeSelectLocation === SL_TAIL && this.points.count() === 2)
      ) {
        this.points.insert(
          this.selectedIndex + 1,
          this.points.getPoint(this.selectedIndex + 1).copy(),
        );
        this.originPoints.insert(
          this.selectedIndex + 1,
          this.points.getPoint(this.selectedIndex + 1).copy(),
        );
      }
      ...
      if (this.selectedIndex > -1) {
        ...
        // Move skeleton
        if (
          op.x <= (_ref4 = p.x + delta.x) &&
          _ref4 <= this.gridFitX(canvas, op.x) + canvas.gridFactor.width
        ) {
          delta.x = op.x - p.x;
          p.x = op.x;
        } else {
          delta.x =
            delta.x - (p.x + delta.x - this.gridFitX(canvas, p.x + delta.x));
          p.x = this.gridFitX(canvas, p.x + delta.x);
        }
        if (
          op.y <= (_ref5 = p.y + delta.y) &&
          _ref5 <= this.gridFitY(canvas, op.y) + canvas.gridFactor.height
        ) {
          delta.y = op.y - p.y;
          p.y = op.y;
        } else {
          delta.y =
            delta.y - (p.y + delta.y - this.gridFitY(canvas, p.y + delta.y));
  ...
  endManipulate(editor, canvas, view, dx, dy) {
    ...
    if (this.selectedPtType === GraphicUtils.CT_ELSE) {
      if (view instanceof NodeParasiticView) {
        ...
        p1 = new Point(
          Math.floor((node.left + node.getRight()) / 2),
          Math.floor((node.top + node.getBottom()) / 2),
        );
        p2 = p1.copy();
      ...
      NodeManipulator.prototype.endManipulate.call(
        this,
        editor,
        canvas,
        view,
...
class DiagramEditor extends EventEmitter {
  constructor(config) {
    ...
    this.canvasElement.onmousedown = (e) => {
      ...
      if (e.button === 1) self.middleButtonDown = true;
      ...
      } else if (self.middleButtonDown) {
        // viewpoint move (middle button)
        self.setCursor(CURSOR_MOVE);
        self.downX = e.offsetX;
        self.downY = e.offsetY;
    ...
    this.canvasElement.onmouseup = (e) => {
      ...
      if (e.button === 1) self.middleButtonDown = false;
      var p = DiagramEditor.getMousePos(self.canvasElement, e);
      var evt = new MouseEvent(
        p.x,
        p.y,
    ...
    this.canvasElement.onmousemove = (e) => {
      ...
      evt.middleButtonDown = self.middleButtonDown;
      if (e.metaKey || e.ctrlKey) {
        // viewpoint move
        if (self.leftButtonDown) {
    ...
    this.canvasElement.ondblclick = (e) => {
      const p = DiagramEditor.getMousePos(self.canvasElement, e);
      Coord.coordRevTransform(
        self.canvas.origin,
        self.canvas.zoomFactor,
        GridFactor.NO_GRID,
        p,
      );
      if (self.diagram) {
        const view = self.diagram.getViewAt(self.canvas, p.x, p.y);
  ...
  getCenter() {
    ...
    const p = new Point(
      sz[0] / this.canvas.ratio / 2,
      sz[1] / this.canvas.ratio / 2,
    );
    Coord.coordRevTransform(
      this.canvas.origin,
  ...
  scrollCenterTo(x, y) {
    ...
    const zsw = sw / this.getZoomScale();
    const zsh = sh / this.getZoomScale();
    const px = Math.round(x - zsw / 2);
    const py = Math.round(y - zsh / 2);
  ...
  drawBackground(g) {
    ...
    if (this.showGrid) {
      ...
      for (let i = 0; i <= wc; i++) {
        const x = p1.x + i * w - (p1.x % w);
        const y = p1.y - (p1.y % h);
        this.canvas.line(x, y, x, p2.y);
      }
      for (let i = 0; i <= wh; i++) {
        const x = p1.x - (p1.x % w);
        const y = p1.y + i * h - (p1.y % h);
        this.canvas.line(x, y, p2.x, y);
      }
